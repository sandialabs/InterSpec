...
/** Shared, de-duplicated accessor for a `SpecMeas`'s automated-search peaks.
 ...
 @param fitPrefs The peak-fit preferences (detector resolution type, skew, ...) to search with.
        If null, falls back to `meas->peakFitDetPrefs()`, and then to `drf->peakFitDetPrefs()`.
