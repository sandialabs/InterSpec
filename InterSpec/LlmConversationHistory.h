...
struct LlmToolCall
{
  ...
  nlohmann::json extra_content; //For like https://aiplatform.googleapis.com/v1beta1/projects/interspec-llm/locations/global/endpoints/openapi/chat/completions
  std::optional<std::chrono::milliseconds> executionDuration; // How long the tool took to execute

  /** Optional image content from a tool result (e.g., spectrum screenshot).
   Runtime-only; not serialized to XML to avoid bloating saved files.
   */
  struct ImageContent
  {
  ...
  std::optional<ImageContent> imageContent;

  /** Visual content attached to a tool result for display in the UI.
   These are NOT sent to the LLM - they are display-only visualizations
   shown to the user in the LlmInteractionDisplay below the text result.
   A single tool call may produce multiple visuals.
   Runtime-only; not serialized to XML to avoid bloating saved files.
   */
  struct VisualContent
  {
    enum class VisualType {
      ChiSquaredPlot,      // ShieldingSourceFitPlot - chi2/mult vs energy
      ShieldingDiagram2D,  // Shielding geometry cross-section
      SpectrumChart,       // Mini spectrum chart with peaks
      Svg,                 // Raw SVG markup
      Html                 // HTML content (rendered in sandboxed iframe)
    };

    VisualType type;
    std::string data;            // JSON data, SVG markup, or HTML content (type-dependent)
    std::string title;           // Optional display title
    std::string description;     // Optional text description shown below the title
    int suggestedWidthPx = 0;    // 0 = use type-specific default
    int suggestedHeightPx = 0;   // 0 = use type-specific default

    /** For SpectrumChart type: spectrum measurement and peak JSON to avoid
     serializing the entire spectrum data to a string.  The measurement
     shared_ptr shares channel/calibration data with what the app displays,
     so memory overhead is minimal.
     */
    std::shared_ptr<const SpecUtils::Measurement> spectrumMeasurement;
    std::string peakJson;        // Peak JSON for D3 chart (from PeakDef::peak_json)
    double xRangeMin = 0.0;      // Energy range for the chart display
    double xRangeMax = 0.0;
  };//struct VisualContent

  std::vector<VisualContent> visualContent;
...
public:
  ...
  enum class Type
  {
    ...
    ConversationSummary   // LLM-generated summary of older conversation exchanges for context management
  };
...
public:
  ...
  /** Signal emitted (on the GUI thread) when a pending async tool call completes.
   The string parameter is the invocationId of the completed tool call.
   */
  Wt::Signal<std::string> &asyncToolCompleted() { return m_asyncToolCompleted; }

protected:
  Wt::Signal<std::string> m_asyncToolCompleted;
...
/** Represents an LLM-generated summary of older conversation history.

 Used for context length management. When the conversation history grows too long,
 older LlmInteraction objects are summarized into a single summary turn. When building
 messages to send to the LLM, the summary replaces the detailed history of the
 summarized conversations.

 Stored as a dedicated LlmInteraction at position 0 of m_conversations, containing
 a single ConversationSummary turn.
 */
class LlmConversationSummary : public LlmInteractionTurn
{
protected:
  std::string m_summary;

  /** Number of LlmInteraction objects that were summarized. */
  size_t m_summarized_count;

  /** Timestamp range of the summarized conversations. */
  std::chrono::system_clock::time_point m_earliest_timestamp;
  std::chrono::system_clock::time_point m_latest_timestamp;

  /** The original conversations that were replaced by this summary.
   Preserved for export/viewing but not sent to the LLM.
   */
  std::vector<std::shared_ptr<LlmInteraction>> m_original_conversations;

public:
  LlmConversationSummary( const std::string &summary,
                           const size_t summarizedCount,
                           const std::chrono::system_clock::time_point earliest,
                           const std::chrono::system_clock::time_point latest,
                           const std::shared_ptr<LlmInteraction> &convo )
    : LlmInteractionTurn( Type::ConversationSummary, convo )
    , m_summary( summary )
    , m_summarized_count( summarizedCount )
    , m_earliest_timestamp( earliest )
    , m_latest_timestamp( latest )
  {
  }

  const std::string &summary() const { return m_summary; }
  void setSummary( const std::string &summary ) { m_summary = summary; }
  size_t summarizedCount() const { return m_summarized_count; }
  const std::chrono::system_clock::time_point &earliestTimestamp() const { return m_earliest_timestamp; }
  const std::chrono::system_clock::time_point &latestTimestamp() const { return m_latest_timestamp; }

  const std::vector<std::shared_ptr<LlmInteraction>> &originalConversations() const { return m_original_conversations; }
  void setOriginalConversations( std::vector<std::shared_ptr<LlmInteraction>> &&convos ) { m_original_conversations = std::move( convos ); }
  std::vector<std::shared_ptr<LlmInteraction>> takeOriginalConversations() { return std::move( m_original_conversations ); }
...
protected:
  ...
  /** Optional image content attached to a user message (e.g., from drag-and-drop).
   Runtime-only; not serialized to XML to avoid bloating saved files.
   */
  std::vector<LlmToolCall::ImageContent> m_imageContent;
...
public:
  ...
  const std::vector<LlmToolCall::ImageContent> &imageContent() const { return m_imageContent; }
  void setImageContent( std::vector<LlmToolCall::ImageContent> images ) { m_imageContent = std::move( images ); }
...
struct LlmInteraction
{
   ...
   */
  static std::shared_ptr<LlmInteraction> createAgent( const Type t, std::string initialMessage, const AgentType a, const LlmConfig * const config );

  static std::shared_ptr<LlmInteraction> createUserInteractionForMainConvo( const std::string &initialMessage );
  static std::shared_ptr<LlmInteraction> createSystemInteractionForMainConvo( const std::string &initialMessage );
...
public:
  ...
  /** Add a user message with optional image content (e.g., from drag-and-drop).
   Same as `addUserMessageToMainConversation(message)`, but also attaches images to the initial request.
   */
  std::shared_ptr<LlmInteraction> addUserMessageToMainConversation( const std::string &message,
  ...
  /** Build the messages array for a MainAgent conversation, including all prior
   ...
   @param convo The current conversation being sent to the LLM
   @param systemPrompt The system prompt to prepend
   @return JSON array of messages ready for the API request
   */
  nlohmann::json buildMessagesForMainAgent(
    const std::shared_ptr<LlmInteraction> &convo,
    const std::string &systemPrompt ) const;

  /** Check if summarization should be triggered based on token counts.

   Uses actual promptTokens from the most recent completed conversation as
   the primary signal. Falls back to character-count estimation (~4 chars/token)
   if token data is unavailable.

   @param contextLengthLimit The model's context window size in tokens.
          If zero, summarization is disabled.
   @return true if context usage exceeds 85% of the limit
   */
  bool shouldSummarize( const size_t contextLengthLimit ) const;

  /** Build a prompt asking the LLM to summarize older conversation exchanges.

   @param keepRecentCount Number of recent exchanges to keep fully intact
   @return The summarization prompt text, or empty string if nothing to summarize
   */
  std::string buildSummarizationPrompt( const size_t keepRecentCount = 5 ) const;

  /** Apply an LLM-generated summary, replacing older conversations.

   Replaces conversations [0..N-keepRecentCount) with a single summary
   LlmInteraction containing a ConversationSummary turn. If a previous
   summary exists at index 0, it is incorporated into the new summary.

   @param summaryText The LLM-generated summary text
   @param keepRecentCount Number of recent exchanges that were kept intact
   */
  void applySummary( const std::string &summaryText, const size_t keepRecentCount = 5 );

  /** Check if the first conversation is a summary placeholder. */
  bool hasSummary() const;

  /** Estimate total token count from all conversations using character lengths.
   Uses ~4 chars per token heuristic as a fallback when actual token counts
   are unavailable.
   */
