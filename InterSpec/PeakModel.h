 ...
 */
  ...
  /** Reads a CSV file generated by PeakCsvResource to peaks that are suitable
   ...
   The peak CSV has no file-level version field.  Where a continuum type's coefficients have changed
   meaning, the `Continuum_Type` cell instead carries a trailing `"(vN)"` - e.g.
   `"BiLinearStepCDF(v3)"` - naming the `PeakContinuum::sm_xmlSerializationVersion` its coefficients
   follow; no suffix means version 2, which is all InterSpec v1.0.14 and earlier ever wrote.  Such
   coefficients are converted on read (see `PeakContinuum::convert_legacy_bilinear_step_cdf`).
   An older InterSpec meeting a tagged type simply fails to match it, and falls back to the
   continuum it derived from the data - which is why the tag goes on the type rather than in a
   comment line an older reader would silently skip.
  ...
  /** Writes the peaks to a CSV file output - e.g., what the user gets when they click on the CSV download on the "Peak Manager" tab.
   ...
   See `csv_to_candidate_fit_peaks` for how the `Continuum_Type` cell carries the coefficient
   convention of the types that have changed one.
   */
