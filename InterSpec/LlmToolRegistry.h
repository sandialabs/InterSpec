...
struct SharedTool {
    ...
    // When asyncExecutor is set, `executor` is ignored.
...
private:
  ...
  /** Whether the LLM supports images. When false, image-producing tools (e.g., get_spectrum_image)
   are excluded from agent tool lists, but remain available for MCP clients. */
...
#define INCLUDE_NOTES_AND_ASSOCIATED_SRCS_WITH_SRC_INFO 1
  ...
  // Spectrum file loading (MCP-only)
  static nlohmann::json executeLoadSpectrumFile(const nlohmann::json& params, InterSpec* interspec);

  // Spectrum image capture
  static void executeGetSpectrumImageAsync(const nlohmann::json& params, InterSpec* interspec,
      std::shared_ptr<LlmInteraction>, LlmConversationHistory*,
      SharedTool::AsyncCallback callback);

  // Time-history tools
  static nlohmann::json executeGetTimeHistoryCountRates(const nlohmann::json& params, InterSpec* interspec);
  static nlohmann::json executeSetDisplayedTimeRanges(const nlohmann::json& params, InterSpec* interspec);
  static nlohmann::json executeGetDisplayedTimeRanges(const nlohmann::json& params, InterSpec* interspec);
  static void executeGetTimeChartImageAsync(const nlohmann::json& params, InterSpec* interspec,
      std::shared_ptr<LlmInteraction>, LlmConversationHistory*,
      SharedTool::AsyncCallback callback);

  /** Builds a JSON summary of a loaded DetectorPeakResponse, shared by load and info tools. */
  static nlohmann::json buildDrfInfoJson( const std::shared_ptr<DetectorPeakResponse> &drf );
