...
/** Simple class to allow the user to select a color.

    Uses the Coloris (https://github.com/mdbassit/Coloris) JavaScript color
    picker, which is lazy-loaded on first user click.
    Currently alpha is not supported, but Coloris does support it for future use.

    Note: Wt's event handling causes a couple non-fatal "Illegal invocation"
    console errors during the initial Coloris lazy-load.  See the comment in
    ColorSelect::loadColoris() for details and the proper fix (prepending a
    try/catch wrapper to coloris.min.js).
 */
...
public:
  enum ColorSelectOptions
  {
    /** If selected, a native HTML5 color chooser will be used, if the client
     supports it; if not the Coloris JS color picker will be used.
     */
    PrefferNative = 0x01,
  ...
  */
  ...
  /** Returns whether a native HTML5 color picker will be used, or Coloris
...
protected:
  ...
  /** Called on first click to lazy-load Coloris JS/CSS and initialize the
      color picker for all ColorSelect widgets on the page.
   */
  void loadColoris();

  /** Initializes this specific widget with Coloris when Coloris is already loaded.
      Called from the constructor when a previous ColorSelect has already loaded Coloris.
   */
...
protected:
  ...
  const bool m_allowNoColor;
  Wt::JSignal<std::string> m_userSelectedColor;
  Wt::JSignal<> m_requestColorisLoad;
  Wt::Signal<Wt::WColor> m_cssColorChanged;
};//class ColorSelect
