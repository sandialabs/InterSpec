...
: ${BOOST_LIBS:="random regex graph random chrono thread signals filesystem system date_time exception program_options serialization timer atomic iostreams"}
...
bootstrapBoost()
{
    ...
    IPHONEOS_DEPLOYMENT_TARGET="" ./bootstrap.sh --with-libraries=$BOOST_LIBS_COMMA
...
buildBoostForIPhoneOS()
{
    ...
    ./bjam -j8 --build-dir=iphone-build -sBOOST_BUILD_USER_CONFIG=$BOOST_SRC/tools/build/example/user-config.jam --stagedir=iphone-build/stage --prefix=$PREFIXDIR abi=aapcs binary-format=mach-o address-model=32 toolset=darwin architecture=arm target-os=iphone macosx-version=iphone-${IPHONE_SDKVERSION} define=_LITTLE_ENDIAN link=static stage > "${LOG}" 2>&1
    if [ $? != 0 ]; then
    ...
    echo " ${LOG}"
    echo "Please stand by..."
    ./bjam -j8 --build-dir=iphone-build -sBOOST_BUILD_USER_CONFIG=$BOOST_SRC/tools/build/example/user-config.jam --stagedir=iphone-build/stage --prefix=$PREFIXDIR abi=aapcs binary-format=mach-o address-model=32 toolset=darwin architecture=arm target-os=iphone macosx-version=iphone-${IPHONE_SDKVERSION} define=_LITTLE_ENDIAN link=static install > "${LOG}" 2>&1
    if [ $? != 0 ]; then
...
scrunchAllLibsTogetherInOneLibPerPlatform()
{
    ...
    #create a set of libs for both sim and devices
    mkdir -p $IOSBUILDDIR/allarch
    for NAME in $BOOST_LIBS; do
      xcrun -sdk iphoneos lipo -create "iphone-build/stage/lib/libboost_$NAME.a" "iphonesim-build/stage/lib/libboost_$NAME.a" -output "$IOSBUILDDIR/allarch/libboost_$NAME.a"
    done
