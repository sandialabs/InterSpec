...
namespace
{
  ...
  // The peak-search/fit API takes the detector peak-fit preferences; these tests only care about
  //  the coarse resolution type, so build a minimal prefs from the tests isHPGe flag.
  shared_ptr<const PeakFitDetPrefs> prefs_for( const bool isHPGe )
  {
    ...
    for( size_t i = 0; i < N; ++i )
    {
      threads.emplace_back( [&,i](){
        shared_future<shared_ptr<const deque<shared_ptr<const PeakDef>>>> f
          = PeakSearchGuiUtils::get_or_launch_automated_search_peaks( meas, samples, spectrum, drf, prefs_for(isHPGe) );
...
namespace
{
  ...
  void run_recovery_check( const string &detector_dir, const string &fg_filename, const bool isHPGe )
  {
    ...
    const vector<shared_ptr<const PeakDef>> fg_peaks
      = ExperimentalAutomatedPeakSearch::search_for_peaks( fg, drf, nullptr, true, prefs_for(isHPGe) );
    const vector<shared_ptr<const PeakDef>> bg_peaks
      = ExperimentalAutomatedPeakSearch::search_for_peaks( bg, drf, nullptr, true, prefs_for(isHPGe) );
    ...
    if( shared_line )
    {
      ...
      const shared_ptr<const deque<shared_ptr<const PeakDef>>> augmented
        = ExperimentalAutomatedPeakSearch::recover_background_peaks_under_foreground(
              fg_peaks, bg, handicapped_ptr, drf, prefs_for(isHPGe) );
    ...
    if( has_peak_near( fg_peaks, source_energy, 3.0 ) && !has_peak_near( bg_peaks, source_energy, 3.0 ) )
    {
      ...
      const shared_ptr<const deque<shared_ptr<const PeakDef>>> augmented
        = ExperimentalAutomatedPeakSearch::recover_background_peaks_under_foreground(
              fg_peaks, bg, base_bg_ptr, drf, prefs_for(isHPGe) );
