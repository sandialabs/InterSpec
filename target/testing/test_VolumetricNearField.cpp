...
BOOST_AUTO_TEST_CASE( VolumetricNearFieldVsTruth )
{
  ...
  // Per-group |model/truth - 1| in percent, so the distribution the allowances are derived from is
  //  REGENERATED by running this test rather than re-derived by hand from the per-row log.  Printed
  //  at the end; only visible at --log_level=message.
  ...
  for( const Scenario &s : scenarios() )
  {
    for( const TruthRow &row : sm_truth )
    {
      ...
      // The FEP window handed to the model is CeeLo's OWN SCORING CONVENTION, not a property of the
      //  detector: the MC tallies a full-energy event on |E_dep - E_src| < kDefaultFepWindowKeV, a
      //  0.75 keV half-width, so those are precisely the forward Compton scatters this truth keeps in
      //  the peak and the model must credit back (mu_rem = mu_total - f_win*mu_Compton).  Crediting
      //  with anything else - FWHM/2 of the anchoring detector, or the mu_total this call used to
      //  imply by leaving the window at its -1 default - is validating against the wrong truth, and
      //  the error grows in proportion to optical depth (Rung9_FepWindowCreditProbe falsification-
      //  tested this: water spheres at 60 keV read -0.36/-1.35/-2.26% for R=1/3/5 cm at FWHM/2 and
      //  +0.18/+0.09/-0.12% at 0.75 keV).
      //
      //  This says NOTHING about the window to use when predicting a REAL measurement, where the peak
      //  region is set by the detector's resolution and the analyst's integration limits.  That is a
      //  different question, it belongs to production (`fep_window_keV` in
      //  ShieldingSourceChi2Fcn::build_volumetric_calculators, from the fitted peak's FWHM/2), and it
      //  must not be answered by agreement with a simulation that scores a fixed 0.75 keV.
      //
      //  The window is the SIXTH argument, after n_rays; landing it in the n_rays slot would silently
      //  change the aperture ray count instead.  One argument covers both shells - build_scenario_calc
      //  applies fep_removal_coefficient to the source matrix AND to the iron shield.
      const double centre = interspec_volumetric_eff( det, s, row.energy_keV, centre_resp,
                                                      -1, ceelo::kDefaultFepWindowKeV );
      ...
      // How much IRON the model has to carry, in mean free paths.  This is the discriminator for the
      //  low-energy groups below, and it is the SHIELD's own depth rather than the total: a deep
      //  source matrix self-selects a surface skin, so getting mu slightly wrong there barely moves
      //  the integral, whereas every ray crosses a shield's full thickness and an error in the
      //  removal coefficient compounds over all of it.  shielded-near-dense @88 keV sits at
      //  tau_total = 5.0 and reads +2.9%; sideon-squat-near-dense @60 keV sits at tau_total = 12.6,
      //  all of it source matrix, and reads +0.08%.
      const double tau_shield = scenario_shield_optical_depth( s, row.energy_keV );

      // Measured 2026-09-04 over the 0.25% bank, centre-anchored, n=144, with the model credited at
      //  the MC's own 0.75 keV FEP window AND carrying the Rayleigh deflection loss on the shield
      //  (both in fep_removal_coefficient; see the call above).  Split by the MECHANISM that limits
      //  each group rather than by convenience; the table is REGENERATED by the summary block at the
      //  end of this test (--log_level=message), so it can never drift from what the code does:
      //
      //      group                                       n   median   p90    max   allowance
      //      far field / wide angle                     36    0.26   2.41   2.87     3.5%
      //      contact, E >= 130 keV                      60    0.44   1.29   1.61     2.5%
      //      contact, E < 130 keV, no shield            36    0.23   0.58   0.83     1.5%
      //      contact, E < 130 keV, Fe shield <= 2 mfp    8    0.53   0.93   0.93     2%
      //      contact, E < 130 keV, Fe shield >  2 mfp    4    0.85   1.03   1.03     2%
      //
      ...
      //
      //  HISTORY OF THE LOW-ENERGY ROWS, because the two shield tiers used to be the worst groups in
      //  the bank and their story is the model's.  Before the FEP-window fix these 48 rows were two
      //  groups at 4% and 7%.  Crediting at the MC's own window collapsed the unshielded ones from
      //  max 2.91% to max 0.83%, and left the shielded ones a clean monotone function of how much
      //  iron the model carried - +0.3% at 122 keV (tau_shield 0.83), +2.7% at 88 keV (1.4),
      //  +10.2% at 60 keV (4.2), always HIGH, and still +4.7% at 60 keV with NO credit at all.  That
      //  last term was RAYLEIGH DEFLECTION: InterSpec's mu excludes coherent scatter, i.e. treats a
      //  Rayleigh-scattered photon as undeflected, but in Fe at 60 keV half of them scatter past
      //  20 degrees, and through a THICK layer that deflection lengthens the remaining path so a
      //  quarter of them are absorbed after all (9% of the peak behind 0.5 cm Fe - the whole of the
      //  over-read; a point-behind-steel depth sweep that was QUADRATIC in tau told it apart from any
      //  path or mu error).  With the term, shielded-near-light reads -1% at every energy from 60 to
      //  1332 keV, i.e. the transfer's own floor, and the two shield tiers sit where the unshielded
      //  rows do.  They are kept as separate groups so a regression in the shield term shows up as
      //  the shield term, not as a smear over 48 rows.
      //
      //  If this fails, the volume integral, the per-ray kernel, or the shield removal coefficient
      //  moved.  That is the finding; do not widen it.
      ...
      {
        const string group = !contact ? "far field / wide angle"
                           : (!low_energy ? "contact, E >= 130 keV"
                           : ((tau_shield <= 0.0) ? "contact, E < 130 keV, no shield"
                           : ((tau_shield <= 2.0) ? "contact, E < 130 keV, Fe shield <= 2 mfp"
                                                  : "contact, E < 130 keV, Fe shield >  2 mfp")));
        ostringstream what;
        what << s.name << " @" << row.energy_keV;
        groups[group].resid.push_back( { 100.0*fabs(centre/truth - 1.0), what.str() } );
      }
  ...
  // The evidence the allowances above are derived from, regenerated on every run so it can never
  //  drift from the numbers quoted in that comment.  p90 is NEAREST-RANK on the sorted |residual|
  //  list, spelled out so the next person recomputes the same statistic.
  BOOST_TEST_MESSAGE( "" );
  BOOST_TEST_MESSAGE( "  |centre-anchored model / MC truth - 1| by mechanism group (%):" );
  BOOST_TEST_MESSAGE( "  group                                        n    median   p90    max"
                      "   worst row" );
  for( pair<const string,GroupStat> &g : groups )
  {
    vector<pair<double,string>> &v = g.second.resid;
    if( v.empty() )
      continue;
    std::sort( begin(v), end(v) );
    const size_t n = v.size();
    const double median = (n % 2) ? v[n/2].first : 0.5*(v[n/2 - 1].first + v[n/2].first);
    const size_t p90_idx = std::min( n - 1, static_cast<size_t>( std::ceil( 0.9*n ) ) - 1 );

    ostringstream o;
    o << "  " << left << setw(42) << g.first << right << setw(4) << n << fixed << setprecision(2)
      << setw(9) << median << setw(7) << v[p90_idx].first << setw(7) << v.back().first
      << "   " << v.back().second;
...
BOOST_AUTO_TEST_CASE( SurvivalRemovalMuSweep, * boost::unit_test::disabled() )
{
  ...
  // The window is NOT a free parameter, but which value it is pinned to depends on the QUESTION.
  //  Against this MC truth it is CeeLo's own scoring half-width, a CONSTANT 0.75 keV
  //  (kDefaultFepWindowKeV) - those are the scatters the truth keeps in its peak.  When predicting a
  //  REAL measurement it is FWHM/2 of the detector: a GEM35-70 HPGe runs ~0.80 keV FWHM at 122 keV
  //  and ~1.85 keV at 1332 keV, and fitting FWHM = sqrt(a + b*E) to those gives a=0.359, b=0.00230,
  //  i.e. half-widths of 0.35 keV at 60 keV rising to 0.93 keV at 1332 keV.  So the two conventions
  //  differ by roughly a FACTOR OF TWO in credit at 60 keV, which is exactly where the credit is
  //  largest - and Rung9_FepWindowCreditProbe showed the MC one is the right half of that pair here.
  //  Both are swept below so the size of the disagreement stays visible.
...
BOOST_AUTO_TEST_CASE( CrossSectionConsistency )
{
  ...
  double worst = 0.0, worst_rayleigh = 0.0; std::string worst_where, worst_rayleigh_where;

  const std::vector<std::string> names = { scenario_matrix_material(true), scenario_matrix_material(false),
                                           scenario_shield_material(), "Lead" };
  for( const std::string &name : names )
  {
    std::shared_ptr<const Material> m;
    try { m = matdb->material( name ); }catch( std::exception & ){ }
    BOOST_REQUIRE_MESSAGE( m, "No material named '" << name << "'" );
    ...
    for( const double e : { 50.0, 60.0, 80.0, 100.0, 122.0, 150.0, 200.0, 344.0, 661.7, 1332.5, 2614.0 } )
    {
      ...
      // Rayleigh alone, summed the way transmition_length_coefficient sums.
      double mu_rs_is = 0.0;
      ...
      {
        worst_rayleigh = std::fabs(rel_rs);
        std::ostringstream w; w << name << " @" << e << " keV";
        worst_rayleigh_where = w.str();
      }
      ...
      BOOST_TEST_MESSAGE( "    E=" << std::setw(7) << e << " keV   InterSpec=" << std::fixed
          ...
          << "   Rayleigh: InterSpec=" << std::setprecision(5) << std::setw(9) << mu_rs_is
          << "/cm  CeeLo=" << std::setw(9) << xs.mu_rs << "/cm  diff=" << std::showpos
          << std::setprecision(2) << rel_rs << "%" << std::noshowpos );
    }
  }
