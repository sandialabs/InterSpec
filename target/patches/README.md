...
If you plan to compile InterSpec with the relative activity tools enabled (CMake option `USE_REL_ACT_TOOL` - default is on), then you will also need to compile/install http://ceres-solver.org/installation.html#macos

```bash
# Build Eigen, which is required by ceres-solver, and used a few other places
# in InterSpec if its avaiable.  Eigen is a header only library, so we dont
# have to worry about architecture
