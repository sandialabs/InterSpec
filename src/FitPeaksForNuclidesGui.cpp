...
void startFitSources( const bool /*from_advanced_dialog*/ )
{
  ...
  Wt::WText *dev_note = wait_dlg->contents()->addNew<Wt::WText>( WString::tr("fpn-wait-dev-note") );
...
void FitPeaksAdvancedWidget::buildOptionsFromConfig()
{
  ...
  {
    auto roi_sig_z = std::make_unique<NativeFloatSpinBox>();
    m_opt_roi_sig_z = roi_sig_z.get();
    ...
    add_form_row( WString::tr("fpn-opt-roi-sig-z"), roi_sig_z.release(), WString::tr("fpn-opt-tt-roi-sig-z") );
  }
