...
std::string agentTypeToString( AgentType type )
{
  switch( type )
  {
    ...
    case AgentType::SpectrumTimeHistory: return "SpectrumTimeHistory";
    case AgentType::InSitu:          return "InSitu";
    case AgentType::McpServer:       return "McpServer";
  }
...
AgentType stringToAgentType( const std::string &name )
{
  ...
  if( name == "SpectrumTimeHistory" ) return AgentType::SpectrumTimeHistory;
  if( name == "InSitu" )          return AgentType::InSitu;
  if( (name == "McpServer") || (name == "Mcp") ) return AgentType::McpServer;
...
std::shared_ptr<LlmConfig> LlmConfig::load()
{
  ...
  // User files in writableDataDirectory() override default files in staticDataDirectory().
  ...
  {
    LlmApi::ModelInfo stubModel;
    stubModel.name = "AnInvalidMdoel";
    stubModel.maxTokens = 4000;
    stubModel.contextLengthLimit = 128000;

    LlmApi::ApiProvider stubProvider;
    stubProvider.apiEndpoint = "http://localhost:1234556787";
    stubProvider.bearerToken = "AnInvalidBearerToken...";
    stubProvider.models.push_back( std::move( stubModel ) );

    config->llmApi.providers.clear();
    config->llmApi.providers.push_back( std::move( stubProvider ) );
    config->llmApi.activeProviderIndex = 0;
  }
#endif
  ...
  // Load each agent from its own file (e.g., "llm_agent_NuclideId.xml").
  // For each agent, check user writable data dir first, then fall back to static data dir.
  {
    const vector<AgentType> agentTypes = {
      AgentType::MainAgent,
      AgentType::NuclideId,
      AgentType::ActivityFit,
      AgentType::Isotopics,
      AgentType::DeepResearch,
      AgentType::EnergyCalibration,
      AgentType::SpectrumTimeHistory,
      AgentType::InSitu
    };

    for( const AgentType agentType : agentTypes )
    {
      const string filename = "llm_agent_" + agentTypeToString( agentType ) + ".xml";
      const string agentPath = get_config_file_path( filename );

      try
      {
        const vector<AgentConfig> agents = loadAgentsFromFile( agentPath );
        for( AgentConfig agent : agents )
          config->agents.push_back( std::move( agent ) );
      }catch( const std::exception &e )
      {
        cerr << "Failed to load agent config from '" << agentPath << "': " << e.what() << endl;
        throw;
      }
    }//for( const AgentType agentType : agentTypes )
...
std::pair<LlmConfig::LlmApi, LlmConfig::McpServer> LlmConfig::loadApiAndMcpConfigs( const std::string &llmConfigPath )
{
  try
  {
    ...
    {// Begin load LLM API settings
      ...
      static_assert( LlmApi::sm_xmlSerializationVersion == 1,
                    "needs to be updated for new serialization version." );

      // check_xml_version throws only if version > required, so version 0 is accepted
      XmlUtils::check_xml_version( llmApiNode, LlmApi::sm_xmlSerializationVersion );

      const int apiVersion = XmlUtils::get_int_attribute( llmApiNode, "version" );
      ...
      // Helper lambda to parse a "reasoning" attribute from a Model node into a variant
      auto parseReasoningAttr = []( const rapidxml::xml_node<char> *node ) -> std::variant<bool, LlmApi::ReasoningEffort>
      {
        const rapidxml::xml_attribute<char> * const reasoningAttr = XML_FIRST_ATTRIB( node, "reasoning" );
        if( !reasoningAttr )
          return false;
        ...
        if( (reasoning_str == "true") || (reasoning_str == "1") || (reasoning_str == "yes") )
          return true;
        if( (reasoning_str == "false") || (reasoning_str == "0") || (reasoning_str == "no") )
          return false;
        if( reasoning_str == "low" )
          return LlmApi::ReasoningEffort::low;
        if( reasoning_str == "medium" )
          return LlmApi::ReasoningEffort::medium;
        if( reasoning_str == "high" )
          return LlmApi::ReasoningEffort::high;
      ...
      {
        string value_str = SpecUtils::xml_value_str( node );
        ...
        return std::nullopt;
      };//parseTemperatureNode lambda

      if( apiVersion == 0 )
      {
        // Version 0: flat structure - wrap into a single ApiProvider with a single ModelInfo
        LlmApi::ApiProvider provider;
        provider.apiEndpoint = XmlUtils::get_string_node_value( llmApiNode, "ApiEndpoint" );
        provider.bearerToken = XmlUtils::get_string_node_value( llmApiNode, "BearerToken" );

        LlmApi::ModelInfo modelInfo;
        const rapidxml::xml_node<char> * const modelNode = XML_FIRST_NODE( llmApiNode, "Model" );
        modelInfo.name = SpecUtils::xml_value_str( modelNode );
        if( modelNode )
          modelInfo.reasoning = parseReasoningAttr( modelNode );

        modelInfo.maxTokens = XmlUtils::get_int_node_value( llmApiNode, "MaxTokens" );
        modelInfo.contextLengthLimit = XmlUtils::get_int_node_value( llmApiNode, "ContextLengthLimit" );

        const rapidxml::xml_node<char> * const tempNode = XML_FIRST_NODE( llmApiNode, "ModelTemperature" );
        modelInfo.temperature = parseTemperatureNode( tempNode );

        // v0 had SupportsImages as a child element of LlmApi
        const rapidxml::xml_node<char> * const supportsImagesNode = XML_FIRST_NODE( llmApiNode, "SupportsImages" );
        string si_str = SpecUtils::xml_value_str( supportsImagesNode );
        SpecUtils::trim( si_str );
        if( !si_str.empty() )
          modelInfo.supportsImages = (si_str == "true" || si_str == "1");

        provider.models.push_back( std::move( modelInfo ) );
      ...
      else
      {
        ...
        for( const rapidxml::xml_node<char> *provNode = XML_FIRST_NODE( llmApiNode, "ApiProvider" );
             provNode;
             provNode = XML_NEXT_TWIN( provNode ) )
        {
          LlmApi::ApiProvider provider;
          provider.apiEndpoint = XmlUtils::get_string_node_value( provNode, "ApiEndpoint" );
          provider.bearerToken = XmlUtils::get_string_node_value( provNode, "BearerToken" );

          // Check "active" attribute on provider
          const rapidxml::xml_attribute<char> * const activeProvAttr = XML_FIRST_ATTRIB( provNode, "active" );
          string active_str = SpecUtils::xml_value_str( activeProvAttr );
          ...
          for( const rapidxml::xml_node<char> *modelNode = XML_FIRST_NODE( provNode, "Model" );
               modelNode;
               modelNode = XML_NEXT_TWIN( modelNode ) )
          {
            LlmApi::ModelInfo modelInfo;
            ...
            const rapidxml::xml_attribute<char> * const imgAttr = XML_FIRST_ATTRIB( modelNode, "supportsImages" );
            if( imgAttr )
            {
              string img_str = SpecUtils::xml_value_str( imgAttr );
              SpecUtils::trim( img_str );
              modelInfo.supportsImages = (img_str == "true" || img_str == "1");
            }

            // Parse requireToolInStateMachine attribute (optional, defaults to false)
            const rapidxml::xml_attribute<char> * const reqToolAttr
              = XML_FIRST_ATTRIB( modelNode, "requireToolInStateMachine" );
            if( reqToolAttr )
            {
              string rt_str = SpecUtils::xml_value_str( reqToolAttr );
            ...
            if( maxTokNode )
            {
              string tok_str = SpecUtils::xml_value_str( maxTokNode );
              SpecUtils::trim( tok_str );
              if( !tok_str.empty() )
              {
                int val;
                if( SpecUtils::parse_int( tok_str.c_str(), tok_str.size(), val ) )
                  modelInfo.maxTokens = val;
              }
            }

            const rapidxml::xml_node<char> * const ctxNode = XML_FIRST_NODE( modelNode, "ContextLengthLimit" );
            if( ctxNode )
            {
              string ctx_str = SpecUtils::xml_value_str( ctxNode );
              SpecUtils::trim( ctx_str );
              if( !ctx_str.empty() )
              {
                int val;
                if( SpecUtils::parse_int( ctx_str.c_str(), ctx_str.size(), val ) )
                  modelInfo.contextLengthLimit = val;
              }
            }

            const rapidxml::xml_node<char> * const tempNode = XML_FIRST_NODE( modelNode, "ModelTemperature" );
            modelInfo.temperature = parseTemperatureNode( tempNode );

            // Check "active" attribute on model
            const rapidxml::xml_attribute<char> * const activeModelAttr = XML_FIRST_ATTRIB( modelNode, "active" );
            string model_active_str = SpecUtils::xml_value_str( activeModelAttr );
            SpecUtils::trim( model_active_str );
            const bool isActiveModel = (model_active_str == "true" || model_active_str == "1");

            if( isActiveModel && !foundActiveModel )
            {
              provider.activeModelIndex = provider.models.size();
      ...
      SpecUtils::trim( value_str );
      llmApi.compactionSystemPrompt = value_str;
