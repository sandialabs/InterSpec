...
struct RelActAutoCostFcn /* : ROOT::Minuit2::FCNBase() */
{
  ...
  m_nanoseconds_spent_in_eval( size_t(0) )
  {
    ...
    if( !drf && (m_options.rel_eff_eqn_type == RelActCalc::RelEffEqnForm::FramPhysicalModel) )
    {
  ...
  size_t number_residuals() const
  {
    ...
    size_t num_resids = (m_options.rel_eff_eqn_type != RelActCalc::RelEffEqnForm::FramPhysicalModel);
    for( const auto &r : m_energy_ranges )
                                                        ...
                                                        )
  {
    ...
    try
    {
      ...
      auto functor = new RelActAutoCostFcn( options, energy_ranges, nuclides,
    ...
    size_t num_rel_eff_par = options.rel_eff_eqn_order + 1;
    if( solution.m_rel_eff_form == RelActCalc::RelEffEqnForm::FramPhysicalModel )
      num_rel_eff_par = 2 + 2*options.phys_model_external_atten.size() + 2;
...
//        constant_parameters.push_back( static_cast<int>( fwhm_start + i ) );
    ...
    assert( (options.rel_eff_eqn_order != 0)
           || (solution.m_rel_eff_form == RelActCalc::RelEffEqnForm::FramPhysicalModel) );
    ...
    switch( solution.m_rel_eff_form )
    {
      ...
      case RelActCalc::RelEffEqnForm::FramEmpirical:
      {
        ...
        if( (options.rel_eff_eqn_type == RelActCalc::RelEffEqnForm::LnX)
           || (options.rel_eff_eqn_type == RelActCalc::RelEffEqnForm::LnY) )
        {
      ...
      case RelActCalc::RelEffEqnForm::FramPhysicalModel:
      {
        const auto set_par = [&parameters]( const size_t start_ind, const RelActCalc::PhysicalModelShieldInput &opt ){
          ...
          parameters[an_index] = 0.0;
          if( !opt.material )
          {
            if( opt.fit_atomic_number
               && ((opt.lower_fit_atomic_number < 1) || (opt.upper_fit_atomic_number > 98)) )
              throw runtime_error( "Self-atten AN is invalid" );
          
            if( (opt.atomic_number >= 1.0) && (opt.atomic_number <= 98.0) )
              parameters[an_index] = opt.atomic_number;
            else
              parameters[an_index] = 0.5*(opt.lower_fit_atomic_number + opt.upper_fit_atomic_number);
          }//if( opt.material ) / else
          
          parameters[ad_index] = opt.fit_areal_density / PhysicalUnits::g_per_cm2;
        };//set_par lambda
        ...
        parameters[b_index] = 0.0; //(energy/1000)^b - start b at 0, so term is 1.0
        parameters[c_index] = 1.0; //c^(1000/energy) - start c at 1, so term is 1
    ...
    try
    {
      ...
      */
      ...
      if( manual_rel_eff_order != options.rel_eff_eqn_order )
      {
        size_t num_rel_eff_coefs = options.rel_eff_eqn_order + 1;
        if( options.rel_eff_eqn_type == RelActCalc::RelEffEqnForm::FramPhysicalModel )
          num_rel_eff_coefs = (2 + 2*options.phys_model_external_atten.size() + 2);
      ...
      if( manual_solution.m_input.eqn_form != RelActCalc::RelEffEqnForm::FramPhysicalModel )
      {
        ...
        for( size_t i = 0; i < num_rel_eff_par; ++i )
      ...
      }else
      {
        ...
        if( options.phys_model_self_atten )
        {
          ...
          if( options.phys_model_self_atten->fit_atomic_number )
          {
            assert( manual_solution.m_rel_eff_eqn_coefficients.size() > manual_index );
            parameters[rel_eff_start + 0] = manual_solution.m_rel_eff_eqn_coefficients.at(manual_index);
            manual_index += 1;
          }
          
          assert( manual_solution.m_rel_eff_eqn_coefficients.size() > manual_index );
          parameters[rel_eff_start + 1] = manual_solution.m_rel_eff_eqn_coefficients.at(manual_index);
          manual_index += 1;
          
          for( size_t i = 0; i < options.phys_model_external_atten.size(); ++i )
          {
            const auto &ext_att = options.phys_model_external_atten[i];
            assert( ext_att );
            if( !ext_att )
              continue;
            ...
            {
              assert( manual_solution.m_rel_eff_eqn_coefficients.size() > manual_index );
              parameters[rel_eff_start + 2 + 2*i + 0] = manual_solution.m_rel_eff_eqn_coefficients.at(manual_index);
              manual_index += 1;
            }
            
            assert( manual_solution.m_rel_eff_eqn_coefficients.size() > manual_index );
            parameters[rel_eff_start + 2 + 2*i + 1] = manual_solution.m_rel_eff_eqn_coefficients.at(manual_index);
            manual_index += 1;
          }//for( loop over options.phys_model_external_atten )
          
          const size_t b_index = rel_eff_start + 2 + 2*options.phys_model_external_atten.size() + 0;
          const size_t c_index = rel_eff_start + 2 + 2*options.phys_model_external_atten.size() + 1;
...
#warning "if statistically significant peaks across less than ~100 keV, then set fit_energy_dep to false"
    ...
    for( size_t nuc_num = 0; nuc_num < nuclides.size(); ++nuc_num )
    {
      ...
      if( fit_age )
      {
        ...
        upper_bounds[age_index] = max_age;
      }else
      {
    ...
    { 
      ...
      setup_physical_model_shield_par( lower_bounds, upper_bounds, constant_parameters, pars, rel_eff_start, self_atten_opt );
    ...
    if( options.rel_eff_eqn_type == RelActCalc::RelEffEqnForm::FramPhysicalModel )
    {
      assert( solution.m_rel_eff_coefficients.size() == (2 + 2*options.phys_model_external_atten.size() + 2) );
      ...
      -> RelActCalcAuto::RelActAutoSolution::PhysicalModelFitInfo::ShieldInfo {
        ...
        if( input.fit_atomic_number )
        {
          ...
          answer.atomic_number = solution.m_rel_eff_coefficients[start_index + 0] * RelActCalc::ns_an_ceres_mult;
          assert( (answer.atomic_number >= 1.0) && (answer.atomic_number <= 98.0) );
          
          if( uncertainties.size() > (rel_eff_start + start_index) )
            answer.atomic_number_uncert = uncertainties[rel_eff_start + start_index + 0];
        }else if( !input.material )
        {
          assert( (solution.m_rel_eff_coefficients[start_index + 0] * RelActCalc::ns_an_ceres_mult)
                 == input.atomic_number );
        ...
        {
          answer.areal_density = solution.m_rel_eff_coefficients[start_index + 1] * PhysicalUnits::g_per_cm2;
          assert( answer.areal_density >= 0.0 );
          
          if( uncertainties.size() > (rel_eff_start + start_index + 1) )
            answer.areal_density_uncert = uncertainties[rel_eff_start + start_index + 1]; //or sqrt( m_rel_eff_covariance[start_index + 1][start_index + 1] )
        }else
        {
          assert( solution.m_rel_eff_coefficients[start_index + 1] == (input.areal_density / PhysicalUnits::g_per_cm2) );
      ...
      if( options.phys_model_self_atten
         && (options.phys_model_self_atten->material
             || ((options.phys_model_self_atten->atomic_number >= 1.0) && (options.phys_model_self_atten->atomic_number <= 98.0))
             || options.phys_model_self_atten->fit_atomic_number ) )
      {
        phys_model_result.self_atten = get_shield_info( *options.phys_model_self_atten, 0 );
      }//if( options.phys_model_self_atten )
      
      for( size_t ext_ind = 0; ext_ind < options.phys_model_external_atten.size(); ++ext_ind )
      {
        auto res = get_shield_info( *options.phys_model_external_atten[ext_ind], 2 + 2*ext_ind );
      ...
      if( options.phys_model_use_hoerl )
      {
        ...
        const size_t b_index = 2 + 2*options.phys_model_external_atten.size();
        const size_t c_index = b_index + 1;
        assert( c_index < solution.m_rel_eff_coefficients.size() );
        
        phys_model_result.hoerl_b = solution.m_rel_eff_coefficients[b_index];
        phys_model_result.hoerl_c = solution.m_rel_eff_coefficients[c_index];
        
        if( uncertainties.size() > (rel_eff_start + b_index) )
          phys_model_result.hoerl_b_uncert = uncertainties[rel_eff_start + b_index];
        if( uncertainties.size() > (rel_eff_start + c_index) )
          phys_model_result.hoerl_b_uncert = uncertainties[rel_eff_start + c_index];
      }//if( options.phys_model_use_hoerl )
      
      solution.m_phys_model_result = std::move(phys_model_result);
  ...
  vector<pair<double,double>> cluster_photopeaks( const double cluster_num_sigma, const std::vector<double> &x ) const
  {
    ...
    for( const NucInputGamma &nuc_input : m_nuclides )
    {
      for( const NucInputGamma::EnergyYield &line_info : nuc_input.nominal_gammas )
      {
        ...
        const double rel_act = relative_activity( nuc_input.nuclide, x );
        const double rel_eff = relative_eff( energy, x );
  ...
  size_t num_rel_eff_coefs() const
  {
    if( m_options.rel_eff_eqn_type == RelActCalc::RelEffEqnForm::FramPhysicalModel )
      return (2 + 2*m_options.phys_model_external_atten.size() + 2);
    return (m_options.rel_eff_eqn_order + 1);
  }
  ...
  static PhysModelRelEqnDef make_phys_eqn_input( const RelActCalcAuto::Options &opts,
                                                std::shared_ptr<const DetectorPeakResponse> drf,
                                                ...
                                                const size_t rel_eff_start )
  {
    ...
    const double b = coeffs[b_index]; //(energy/1000)^b
    const double c = coeffs[c_index]; //c^(1000/energy)
  ...
  double relative_eff( const double energy, const std::vector<double> &x ) const
  {
    ...
    const double * const rel_ef_pars = &(x[rel_eff_start_index]);
    ...
    if( m_options.rel_eff_eqn_type == RelActCalc::RelEffEqnForm::FramPhysicalModel )
    {
      ...
      const PhysModelRelEqnDef re_input = make_phys_eqn_input( m_options,
                                                             m_drf, x, rel_eff_start_index );
      
      return RelActCalc::eval_physical_model_eqn( energy, re_input.self_atten,
                                                 re_input.external_attens, re_input.det.get(),
                                                 re_input.hoerl_b, re_input.hoerl_c );
    }//if( m_options.rel_eff_eqn_type == RelActCalc::RelEffEqnForm::FramPhysicalModel )
    
    assert( m_options.rel_eff_eqn_type != RelActCalc::RelEffEqnForm::FramPhysicalModel );
    
    return RelActCalc::eval_eqn( energy, m_options.rel_eff_eqn_type, rel_ef_pars, num_rel_eff_par );
  }//
        ...
        if( (m_options.additional_br_uncert > 0.0) && !m_peak_ranges_with_uncert.empty() )
        {
          ...
          for( size_t range_index = 0; range_index < m_peak_ranges_with_uncert.size(); ++range_index )
          {
            ...
            if( (gamma.energy >= range.first) && (gamma.energy <= range.second) )
            {
              ...
              const double adjvalue = x[m_add_br_uncert_start_index + range_index]/sm_peak_range_uncert_par_offset - 1.0;
  ...
  void eval( const std::vector<T> &x, T *residuals ) const
  {
    ...
    for( size_t roi_index = 0; roi_index < m_energy_ranges.size(); ++roi_index )
    {
      ...
      const PeakContinuum * const continuum = info.peaks.at(0).continuum().get();
      assert( continuum );
      ...
      if( try_better_par )
      {
        mutex *this_roi_mutex = &(roi_mutexes[roi_index]);
        
        const PeaksForEnergyRange &range = peaks_in_ranges[roi_index];
        ...
        double * const this_residual = residuals + residual_start_index;
          ...
          {
            pool.post( [this_roi_mutex, this_residual, &peak, this_nchannel, this_energies](){
              vector<double> counts( this_nchannel, 0.0 );
              peak.gauss_integral( this_energies, counts.data(), this_nchannel );
              
              std::lock_guard<std::mutex> lock( *this_roi_mutex );
              for( size_t i = 0; i < this_nchannel; ++i )
                this_residual[i] += counts[i];
            } );
          }//for( const PeakDef &peak : range.peaks )
          
          // Fill the continuum values
          pool.post( [this_nchannel, this_energies, continuum, this_residual, this_roi_mutex, this](){
            vector<double> counts( this_nchannel, 0.0 );
            for( size_t index = 0; index < this_nchannel; ++index )
            {
              const double x0 = this_energies[index];
              const double x1 = this_energies[index + 1];
              counts[index] += continuum->offset_integral( x0, x1, m_spectrum );
            }
      ...
      }else if( parallelize_over_rois_only )
      {
        ...
        pool.post( [eval_for_roi,roi_index,&peaks_in_ranges](){
          ...
          //const auto end_time = std::chrono::high_resolution_clock::now();
          //std::lock_guard<std::mutex> lock( debugmtx );
          //cout << "For ROI " << roi_index << " spent " << std::chrono::duration_cast<std::chrono::nanoseconds>(end_time - start_time).count() << " nanoseconds" << endl;
        } );
      }else
      {
...
*/
    ...
    if( try_better_par )
    {
      ...
      for( size_t roi_index = 0; roi_index < m_energy_ranges.size(); ++roi_index )
      {
        double * const this_residual = residuals + residual_index;
        const PeaksForEnergyRange &range = peaks_in_ranges[roi_index];
        const size_t first_channel = range.first_channel;
        const size_t nchannels_roi = (range.last_channel - range.first_channel) + 1;
        
        residual_index += nchannels_roi;
        
        pool.post( [first_channel,nchannels_roi,this_residual,this](){
          for( size_t index = 0; index < nchannels_roi; ++index )
          {
            const size_t data_index = first_channel + index;
            const double data_counts = m_channel_counts[data_index];
            const double data_uncert = m_channel_count_uncerts[data_index];
            const double peak_area = this_residual[index];
...
void Options::fromXml( const ::rapidxml::xml_node<char> *parent, MaterialDB *materialDB )
{
  try
  {
    ...
    const rapidxml::xml_node<char> *add_uncert_node = XML_FIRST_NODE( parent, "AddUncert" );
    if( add_uncert_node )
    {
      if( !SpecUtils::parse_double(add_uncert_node->value(), add_uncert_node->value_size(), additional_br_uncert) )
        throw std::runtime_error( "AddUncert node value ('" + SpecUtils::xml_value_str(add_uncert_node) + "') was not a valid float." );
      if( IsNan(additional_br_uncert) || IsInf(additional_br_uncert) )
        throw std::runtime_error( "AddUncert node is NaN or inf." );
...
void RelActAutoSolution::print_html_report( std::ostream &out ) const
{
  ...
  if( m_rel_eff_form != RelActCalc::RelEffEqnForm::FramPhysicalModel )
  {
    rel_eff_eqn_js = RelActCalc::rel_eff_eqn_js_function( m_rel_eff_form, m_rel_eff_coefficients );
  }else
  {
    const RelActAutoCostFcn::PhysModelRelEqnDef input
             = RelActAutoCostFcn::make_phys_eqn_input( m_options, m_drf, m_rel_eff_coefficients, 0 );
                         ...
                         )
{
  ...
  for( ; !stop_iterating && (num_roi_iters < max_roi_adjust_iterations); ++num_roi_iters )
  {
    ...
    RelActAutoCostFcn::PhysModelRelEqnDef phys_model_input;
    if( options.rel_eff_eqn_type == RelActCalc::RelEffEqnForm::FramPhysicalModel )
    {
    ...
    for( const RoiRange &roi : energy_ranges )
    {
      ...
      for( const NuclideRelAct &rel_act : current_sol.m_rel_activities )
      {
        ...
        for( const pair<double,double> &energy_br : rel_act.gamma_energy_br )
        {
          ...
          if( options.rel_eff_eqn_type != RelActCalc::RelEffEqnForm::FramPhysicalModel )
          {
            rel_eff = RelActCalc::eval_eqn( energy, current_sol.m_rel_eff_form,
                                           current_sol.m_rel_eff_coefficients );
          }else
          {
            rel_eff = RelActCalc::eval_physical_model_eqn( energy, phys_model_input.self_atten,
