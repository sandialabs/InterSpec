...
namespace {
  ...
  vector<double> get_number_array( const json& parent, const string &name )
  {
    if( !parent.contains(name) )
      throw runtime_error( "'" + name + "' parameter must be specified." );

    const json &val = parent[name];

    vector<double> answer;

    const auto convert_array = [&]( const json &arr ){
      for( const json &v : arr )
      {
        if( v.is_number() )
        {
          answer.push_back( v.get<double>() );
          continue;
        }

        try
        {
          double num = get_double( v );
        ...
        throw runtime_error( "An element in the '" + name + "' array is not a number." );
      }
    };
    ...
    if( val.is_number() )
    {
      answer.push_back( val.get<double>() );
      return answer;
    }

    if( !val.is_string() )
      throw runtime_error( "The '" + name + "' element should be an array of numbers." );

    const string strval = val.get<string>();

    try
    {
      // TODO: use LlmInterfaceTests::lenientlyParseJson
      nlohmann::json maybe_array = nlohmann::json::parse( strval );
      if( maybe_array.is_array() )
      {
        convert_array(maybe_array);
        return answer;
      }else if( maybe_array.is_number() )
      {
        answer.push_back( maybe_array.get<double>() );
        return answer;
      }
    }catch( ... )
    {
    ...
    try
    {
      const double val = get_double( strval );
...
)
{
  ...
  // Put in a slight delay so user cad
  const int num_ms_delay = 2000;
  ...
  if( num_ms_delay > 0 )
  {
    ...
    Wt::WServer::instance()->schedule( std::chrono::milliseconds(2000), sessionid, [](){
      ...
      if( !app || !viewer )
        return;
      viewer->closeShieldingSourceFit();
      app->triggerUpdate();
    } );
  }else
  {
...
)
{
  ...
  const vector<double> peak_energies = get_number_array( params, "peak_energies" );
  if( peak_energies.empty() )
    throw runtime_error( "peak_energies must not be empty" );
  ...
  for( const double &energy : peak_energies )
  {
    try
    {
      ...
      for( size_t i = 0; i < peaks->size(); ++i )
      {
        ...
        if( std::fabs( peak_energy - energy ) <= tolerance )
        {
          ...
          if( peak->useForShieldingSourceFit() != use_for_fit )
          {
            ...
            peakModel->setData( model_index, Wt::cpp17::any(use_for_fit) );

            json marked_peak;
            marked_peak["energy"] = round_to_decimal_places( peak_energy, 3 );
      ...
      {
        json error;
        error["energy"] = round_to_decimal_places( energy, 3 );
...
)
{
  ...
  for( size_t i = 0; i < config.shieldings.size(); ++i )
  {
    ...
    if( !shield.m_traceSources.empty() )
    {
      ...
      for( const ShieldingSourceFitCalc::TraceSourceInfo &trace : shield.m_traceSources )
      {
        ...
        trace_json["activity"] = round_to_sig_figs( trace.m_activity, 6 );
        ...
        switch( trace.m_type )
        {
          ...
          case GammaInteractionCalc::TraceActivityType::ExponentialDistribution:
            ...
            trace_json["relaxation_distance_cm"] = round_to_sig_figs( trace.m_relaxationDistance, 6 );
            break;
  ...
  for( const ShieldingSourceFitCalc::SourceFitDef &src : config.sources )
  {
    ...
    src_json["activity_bq"] = round_to_sig_figs( src.activity, 6 );
    src_json["fit_activity"] = src.fitActivity;

    if( src.age > 0.0 )
    {
      src_json["age_years"] = round_to_sig_figs( src.age / PhysicalUnits::year, 6 );
  ...
  {
    json peak_json;
    peak_json["energy_kev"] = round_to_decimal_places( peak.energy, 3 );
...
)
{
  ...
  vector<string> modify_warnings;
  GammaInteractionCalc::ShieldSourceConfig modified_config = *state.config;

  shared_ptr<const SpecMeas> meas = interspec->measurment( SpecUtils::SpectrumType::Foreground );
  shared_ptr<const DetectorPeakResponse> detector = meas ? meas->detector() : nullptr;
  ...
  if( operation == "set_distance" )
  {
    ...
    if( detector && detector->isFixedGeometry() )
    {
      if( !distance_str.empty() )
        throw runtime_error( "The current detector efficiency function is for a fixed geometry - you can not set the distance for this setup." );
    }else
    {
      if( distance_str.empty() )
  ...
  }else if( operation == "set_geometry" )
  {
    ...
    if( detector && detector->isFixedGeometry() )
    {
      if( !geometry_str.empty() )
        throw runtime_error( "The current detector efficiency function is for a fixed geometry - you can not set geometry for this setup." );
    }else
    {
      if( geometry_str.empty() )
  ...
  else if( operation == "add_source" )
  {
    ...
    for( const shared_ptr<const PeakDef> &peak : *peaks )
    {
      ...
      if( !peak->useForShieldingSourceFit() )
      {
        Wt::WModelIndex peak_index = peakModel->indexOfPeak(peak);
        Wt::WModelIndex model_index = peakModel->index( peak_index.row(), PeakModel::kUseForShieldingSourceFit );
        peakModel->setData( model_index, Wt::cpp17::any(true) );
        ++num_peaks_marked;
      ...
      if( peaks )
      {
        ...
        for( size_t i = 0; i < peaks->size(); ++i )
        {
          ...
          if( peak && peak->parentNuclide() == nuclide && peak->useForShieldingSourceFit() )
          {
            ...
            peakModel->setData( model_index, Wt::cpp17::any(false) );
            ++num_peaks_unmarked;
          }
        }
  ...
  {
    // Check nuclide is specified and valid
    const string nuclide_str = params.value( "nuclide", string() );
    if( nuclide_str.empty() )
      throw runtime_error( "nuclide parameter is required for modify_source operation" );
    ...
    auto src_it = std::find_if( modified_config.sources.begin(), modified_config.sources.end(),
      [nuclide]( const ShieldingSourceFitCalc::SourceFitDef &s ) {
        return s.nuclide == nuclide;
      });

    if( src_it == modified_config.sources.end() )
      throw runtime_error( "Source '" + nuclide_str + "' not found in configuration" );

    ShieldingSourceFitCalc::SourceFitDef &src_def = *src_it;

    // Helper: remove this nuclide from all trace/intrinsic shielding associations.
    const auto remove_shielding_associations = [&]()
    {
      const SandiaDecay::Element *el = db->element( nuclide->atomicNumber );
      for( ShieldingSourceFitCalc::ShieldingInfo &shield : modified_config.shieldings )
      {
        // Remove from trace sources
        shield.m_traceSources.erase(
          std::remove_if( shield.m_traceSources.begin(), shield.m_traceSources.end(),
            [nuclide]( const ShieldingSourceFitCalc::TraceSourceInfo &t ) {
              return t.m_nuclide == nuclide;
            }),
          shield.m_traceSources.end() );

        // Remove from intrinsic mass-fraction map
        if( el )
        {
          auto el_it = shield.m_nuclideFractions_.find( el );
          if( el_it != shield.m_nuclideFractions_.end() )
          {
            auto &nuc_vec = el_it->second;
            nuc_vec.erase(
              std::remove_if( nuc_vec.begin(), nuc_vec.end(),
                [nuclide]( const std::tuple<const SandiaDecay::Nuclide *, double, bool> &t ) {
                  return std::get<0>(t) == nuclide;
                }),
              nuc_vec.end() );
            if( nuc_vec.empty() )
              shield.m_nuclideFractions_.erase( el_it );
          }//if( el is in map )
        }//if( el )
      }//for( each shielding )
    };//remove_shielding_associations lambda

    // Helper: resolve a shielding layer by index or material name.
    // Returns a pointer into modified_config.shieldings (null on failure throws).
    const auto resolve_shielding = [&]() -> ShieldingSourceFitCalc::ShieldingInfo &
    {
      const bool has_material = params.contains("material") && !params.value("material", string()).empty();
      const bool has_index = params.contains("index");

      if( !has_material && !has_index )
        throw runtime_error( "source_type 'Trace'/'Intrinsic' requires 'material' or 'index' to identify the shielding layer" );
      if( has_material && has_index )
        throw runtime_error( "Specify exactly one of 'material' or 'index', not both" );

      if( has_index )
      {
        const int idx = params.value( "index", -1 );
        if( idx < 0 || idx >= static_cast<int>(modified_config.shieldings.size()) )
          throw runtime_error( "Shielding index " + std::to_string(idx) + " out of range (have "
                               + std::to_string(modified_config.shieldings.size()) + " shielding layers)" );
        return modified_config.shieldings[static_cast<size_t>(idx)];
      }

      // Resolve by material name
      string material = params.value( "material", string() );
      const std::shared_ptr<const MaterialDB> materialDB = MaterialDB::instance();
      if( materialDB )
      {
      ...
      for( size_t i = 0; i < modified_config.shieldings.size(); ++i )
      {
        const ShieldingSourceFitCalc::ShieldingInfo &s = modified_config.shieldings[i];
        if( !s.m_material )
          continue;
        if( s.m_material->name == material || s.m_material->description == material
            || s.m_material->name.find(material) != string::npos )
          matching.push_back(i);
      }

      if( matching.empty() )
        throw runtime_error( "No shielding layer with material '" + material
                             + "' found; use 'index' to select by position" );
      if( matching.size() > 1 )
        throw runtime_error( "Multiple shielding layers match material '" + material
                             + "'; use 'index' to select unambiguously" );
      return modified_config.shieldings[matching[0]];
    };//resolve_shielding lambda

    // --- source_type transition ---
    const string source_type_str = params.value( "source_type", string() );
    if( !source_type_str.empty() )
    {
      if( source_type_str == "Point" )
      {
        if( params.contains("material") || params.contains("index") )
          modify_warnings.push_back( "'material'/'index' ignored when source_type is Point" );

        remove_shielding_associations();
        src_def.sourceType = ShieldingSourceFitCalc::ModelSourceType::Point;
        // Intrinsic sources always have fitActivity=false; reset to true when promoting to Point.
        if( !src_def.fitActivity )
          src_def.fitActivity = true;
      }
      else if( source_type_str == "Trace" )
      {
        if( detector && detector->isFixedGeometry() )
          throw runtime_error( "Current detector efficiency is a fixed-geometry efficiency - you can not define trace sources. The dector efficiency has aready accounted for geometry and attenuation.  You should not be using 'Air' shielding either." );

        ShieldingSourceFitCalc::ShieldingInfo &target = resolve_shielding();
        remove_shielding_associations();

        // Parse optional activity_type
        GammaInteractionCalc::TraceActivityType trace_act_type = GammaInteractionCalc::TraceActivityType::TotalActivity;
        const string activity_type_str = params.value( "activity_type", string() );
        if( !activity_type_str.empty() )
        {
          if( activity_type_str == "total_activity" )
            trace_act_type = GammaInteractionCalc::TraceActivityType::TotalActivity;
          else if( activity_type_str == "per_cm3" )
            trace_act_type = GammaInteractionCalc::TraceActivityType::ActivityPerCm3;
          else if( activity_type_str == "per_gram" )
            trace_act_type = GammaInteractionCalc::TraceActivityType::ActivityPerGram;
          else if( activity_type_str == "exponential" )
            trace_act_type = GammaInteractionCalc::TraceActivityType::ExponentialDistribution;
          else
            throw runtime_error( "Invalid activity_type '" + activity_type_str
                                 + "'; must be per_cm3, total_activity, per_gram, or exponential" );
        }

        ShieldingSourceFitCalc::TraceSourceInfo trace;
        trace.m_nuclide = nuclide;
        trace.m_type = trace_act_type;
        trace.m_fitActivity = true;
        trace.m_relaxationDistance = 0.0f;

        // Default activity depends on distribution type
        switch( trace_act_type )
        {
          case GammaInteractionCalc::TraceActivityType::TotalActivity:
            trace.m_activity = (src_def.activity > 0.0) ? src_def.activity : 1.0 * PhysicalUnits::microCi;
            break;
          case GammaInteractionCalc::TraceActivityType::ActivityPerCm3:
          case GammaInteractionCalc::TraceActivityType::ExponentialDistribution:
            trace.m_activity = 1.0 * PhysicalUnits::bq / PhysicalUnits::cm3;
            break;
          case GammaInteractionCalc::TraceActivityType::ActivityPerGram:
            trace.m_activity = 1.0 * PhysicalUnits::bq / PhysicalUnits::gram;
            break;
          case GammaInteractionCalc::TraceActivityType::NumTraceActivityType:
            break;
        }

        // Relaxation distance: only meaningful for ExponentialDistribution
        if( trace_act_type == GammaInteractionCalc::TraceActivityType::ExponentialDistribution )
        {
          const string relax_str = params.value( "relaxation_distance", string() );
          if( !relax_str.empty() )
            trace.m_relaxationDistance = static_cast<float>( parse_distance_string( relax_str ) );
          else
            trace.m_relaxationDistance = static_cast<float>( 1.0 * PhysicalUnits::cm );
        }

        target.m_traceSources.push_back( trace );
        src_def.sourceType = ShieldingSourceFitCalc::ModelSourceType::Trace;
        src_def.activity = trace.m_activity;
        src_def.fitActivity = true;
      }
      else if( source_type_str == "Intrinsic" )
      {
        if( params.contains("activity") )
          throw runtime_error( "'activity' cannot be set when source_type is Intrinsic" );
        if( params.contains("fit_activity") )
          throw runtime_error( "'fit_activity' cannot be set when source_type is Intrinsic" );

        ShieldingSourceFitCalc::ShieldingInfo &target = resolve_shielding();

        // Verify the nuclide's element exists in the target material
        const SandiaDecay::Element *el = db->element( nuclide->atomicNumber );
        if( !el )
          throw runtime_error( "Could not resolve element for nuclide '" + nuclide_str + "'" );
        if( target.m_material )
        {
          bool element_in_material = false;
          for( const auto &mat_el : target.m_material->elements )
            element_in_material = element_in_material || (mat_el.first == el);
          for( const auto &mat_nuc : target.m_material->nuclides )
            element_in_material = element_in_material || (mat_nuc.first == nuclide);
          if( !element_in_material )
            throw runtime_error( "Nuclide '" + nuclide_str + "' element is not present in material '"
                                 + target.m_material->name + "'" );
        }

        remove_shielding_associations();

        // Default mass fraction is a small value; fit flag is true
        const double default_fraction = 1.0e-6;
        target.m_nuclideFractions_[el].emplace_back( nuclide, default_fraction, true );

        src_def.sourceType = ShieldingSourceFitCalc::ModelSourceType::Intrinsic;
        src_def.fitActivity = false; // Intrinsic sources never fit activity
      }
      else
      {
        throw runtime_error( "Invalid source_type '" + source_type_str
                             + "'; must be Point, Trace, or Intrinsic" );
      }
    }//if( !source_type_str.empty() )

    // --- activity_type update for an already-Trace source (source_type not changing) ---
    if( source_type_str.empty() && params.contains("activity_type")
        && src_def.sourceType == ShieldingSourceFitCalc::ModelSourceType::Trace )
    {
      const string activity_type_str = params.value( "activity_type", string() );
      ...
      float relax_dist = 0.0f;
      if( trace_act_type == GammaInteractionCalc::TraceActivityType::ExponentialDistribution )
      {
        const string relax_str = params.value( "relaxation_distance", string() );
        if( !relax_str.empty() )
          relax_dist = static_cast<float>( parse_distance_string( relax_str ) );
        else
          relax_dist = static_cast<float>( 1.0 * PhysicalUnits::cm );
      }
      ...
      for( ShieldingSourceFitCalc::ShieldingInfo &shield : modified_config.shieldings )
      {
        for( ShieldingSourceFitCalc::TraceSourceInfo &trace : shield.m_traceSources )
        {
          if( trace.m_nuclide == nuclide )
          {
            trace.m_type = trace_act_type;
            if( trace_act_type == GammaInteractionCalc::TraceActivityType::ExponentialDistribution )
              trace.m_relaxationDistance = relax_dist;
          }
        }
      }
    }//if( updating activity_type of already-Trace source )
    ...
    if( !age_str.empty() )
    {
      ...
      src_def.age = parse_age_string( age_str );
    }

    // --- fit_age ---
    if( params.contains("fit_age") )
    {
      const bool fit = get_boolean( params, "fit_age" );
      const bool ageAllowed = !PeakDef::ageFitNotAllowed( nuclide );

      if( src_def.ageDefiningNuc && (src_def.ageDefiningNuc != nuclide) )
      {
        // TODO: in principle, if modified_config.options.same_age_isotopes, we should just set set the age of the controlling nuclide, and add a warning in `result["warnings"]`.
        throw runtime_error( "Age can not be set to fit for '" + nuclide_str + "' - it age is controlled by '"
                            + src_def.ageDefiningNuc->symbol + "' (see same_age_isotopes option, or set age for '"
                            + src_def.ageDefiningNuc->symbol + "')." );
      }

      if( fit && !ageAllowed )
        throw runtime_error( "Age can not be fit for '" + nuclide_str + "' - its peak amplitudes dont change enough as a function of age." );

      src_def.fitAge = fit;
    }

    // --- activity (Point and Trace only) ---
    const string activity_str = params.value( "activity", string() );
    if( !activity_str.empty() )
    {
      if( src_def.sourceType == ShieldingSourceFitCalc::ModelSourceType::Point )
      {
        src_def.activity = parse_activity_string( activity_str );
      }else if( src_def.sourceType == ShieldingSourceFitCalc::ModelSourceType::Trace )
      {
        const pair<GammaInteractionCalc::TraceActivityType, double> activity_type_val = parse_trace_activity_string( activity_str );
        ...
        for( ShieldingSourceFitCalc::ShieldingInfo &shield : modified_config.shieldings )
        {
          for( ShieldingSourceFitCalc::TraceSourceInfo &trace : shield.m_traceSources )
          {
            if( trace.m_nuclide == nuclide )
            {
              trace.m_activity = activity_type_val.second;
              trace.m_type = activity_type_val.first;
            }
          }
        }//for( ShieldingSourceFitCalc::ShieldingInfo &shield : modified_config.shieldings )
      }else
      {
        assert( src_def.sourceType == ShieldingSourceFitCalc::ModelSourceType::Intrinsic );
        throw runtime_error( "'activity' cannot be set for Intrinsic sources" );
      }
    }//if( !activity_str.empty() )

    // --- fit_activity (Point and Trace only) ---
    if( params.contains("fit_activity") )
    {
      if( src_def.sourceType == ShieldingSourceFitCalc::ModelSourceType::Intrinsic )
        throw runtime_error( "'fit_activity' cannot be set for Intrinsic sources" );

      const bool new_fit_activity = get_boolean( params, "fit_activity" );
      src_def.fitActivity = new_fit_activity;

      if( src_def.sourceType == ShieldingSourceFitCalc::ModelSourceType::Trace )
      {
        for( ShieldingSourceFitCalc::ShieldingInfo &shield : modified_config.shieldings )
        {
          for( ShieldingSourceFitCalc::TraceSourceInfo &trace : shield.m_traceSources )
          {
            if( trace.m_nuclide == nuclide )
              trace.m_fitActivity = new_fit_activity;
          }
        }
      }
    }//if( params.contains("fit_activity") )
    ...
    const auto source_type_name = []( ShieldingSourceFitCalc::ModelSourceType t ) -> string
    {
      switch( t )
      {
        case ShieldingSourceFitCalc::ModelSourceType::Point:     return "Point";
        case ShieldingSourceFitCalc::ModelSourceType::Trace:     return "Trace";
        case ShieldingSourceFitCalc::ModelSourceType::Intrinsic: return "Intrinsic";
      }
      return "Unknown";
    };

    result["nuclide"] = nuclide_str;
    result["source_type"] = source_type_name( src_def.sourceType );
    result["age_s"] = src_def.age;
    result["fit_age"] = src_def.fitAge;

    if( src_def.sourceType != ShieldingSourceFitCalc::ModelSourceType::Intrinsic )
    {
      result["activity_bq"] = src_def.activity / PhysicalUnits::bq;
      result["fit_activity"] = src_def.fitActivity;
    }

    // Report shielding assignment for Trace/Intrinsic
    if( src_def.sourceType == ShieldingSourceFitCalc::ModelSourceType::Trace
        || src_def.sourceType == ShieldingSourceFitCalc::ModelSourceType::Intrinsic )
    {
      const SandiaDecay::Element *el = db->element( nuclide->atomicNumber );
      for( size_t i = 0; i < modified_config.shieldings.size(); ++i )
      {
        const ShieldingSourceFitCalc::ShieldingInfo &shield = modified_config.shieldings[i];
        bool found = false;
        for( const ShieldingSourceFitCalc::TraceSourceInfo &trace : shield.m_traceSources )
        {
          if( trace.m_nuclide == nuclide )
          {
            found = true;
            // Report trace activity type
            switch( trace.m_type )
            {
              case GammaInteractionCalc::TraceActivityType::TotalActivity:
                result["activity_type"] = "total_activity"; break;
              case GammaInteractionCalc::TraceActivityType::ActivityPerCm3:
                result["activity_type"] = "per_cm3"; break;
              case GammaInteractionCalc::TraceActivityType::ActivityPerGram:
                result["activity_type"] = "per_gram"; break;
              case GammaInteractionCalc::TraceActivityType::ExponentialDistribution:
                result["activity_type"] = "exponential";
                result["relaxation_distance_cm"] = trace.m_relaxationDistance / PhysicalUnits::cm;
                break;
              case GammaInteractionCalc::TraceActivityType::NumTraceActivityType:
                break;
            }
        ...
        if( !found && el )
        {
          const auto el_it = shield.m_nuclideFractions_.find(el);
          if( el_it != shield.m_nuclideFractions_.end() )
          {
            for( const auto &nuc_info : el_it->second )
              found = found || (std::get<0>(nuc_info) == nuclide);
          }
        }
        if( found )
        {
          result["shielding_index"] = i;
          result["shielding_material"] = shield.m_material ? shield.m_material->name : string("Generic");
          break;
        }
      }//for( each shielding )
    }//if( Trace or Intrinsic )
  ...
  if( !modify_warnings.empty() )
  {
    json warnings_arr = json::array();
    for( const string &w : modify_warnings )
      warnings_arr.push_back(w);
    result["warnings"] = warnings_arr;
  }
...
)
{
  ...
  if( src.isTraceSource )
  {
    ...
    activity_json["display_bq"] = round_to_sig_figs( src.traceSrcDisplayAct / PhysicalUnits::bq, 6 );
    activity_json["display_ci"] = round_to_sig_figs( src.traceSrcDisplayAct / PhysicalUnits::ci, 6 );
    ...
    activity_json["total_bq"] = round_to_sig_figs( src.activity / PhysicalUnits::bq, 6 );
    activity_json["total_ci"] = round_to_sig_figs( src.activity / PhysicalUnits::ci, 6 );
    ...
    {
      activity_json["display_uncertainty_bq"] = round_to_sig_figs( src.traceSrcDisplayActUncertainty / PhysicalUnits::bq, 6 );
      activity_json["display_uncertainty_ci"] = round_to_sig_figs( src.traceSrcDisplayActUncertainty / PhysicalUnits::ci, 6 );
      ...
      activity_json["total_uncertainty_bq"] = round_to_sig_figs( src.activityUncertainty / PhysicalUnits::bq, 6 );
      activity_json["total_uncertainty_ci"] = round_to_sig_figs( src.activityUncertainty / PhysicalUnits::ci, 6 );
      ...
      activity_json["uncertainty_percent"] = round_to_sig_figs( display_uncert_percent, 6 );
    }//if( src.activityIsFit )

    // Add nuclide mass
    activity_json["nuclide_mass_grams"] = round_to_sig_figs( src.nuclideMass / PhysicalUnits::gram, 6 );
...
)
{
  ...
  std::shared_ptr<ShieldingSourceFitCalc::ModelFitResults> fit_results = display->doModelFit( false, false );
  ...
  if( fit_results )
  {
    ...
    // Add visual content for display in LlmInteractionDisplay (not sent to LLM)
    try
    {
      json visuals = json::array();

      // Chi-squared / multiplier plot
      {
        json chiVis;
        chiVis["type"] = "chi_squared_plot";
        chiVis["title"] = "Fit Quality";
        chiVis["description"] = "Chi-squared residuals and observed/expected multiplier vs energy";
        chiVis["data"] = json::parse( ShieldingSourceFitPlot::jsonForData( *fit_results ) );
        chiVis["heightPx"] = 185;
        chiVis["widthPx"] = 250;
        visuals.push_back( std::move( chiVis ) );
      }

      // Shielding diagram
      {
        const vector<ShieldingSourceFitCalc::ShieldingInfo> shieldings(
          fit_results->final_shieldings.begin(), fit_results->final_shieldings.end() );

        const double det_diam = detector ? detector->detectorDiameter() : 50.0 * PhysicalUnits::mm;

        const string diagJson = createShieldingDiagramJson(
          shieldings, fit_results->fit_src_info, fit_results->geometry,
          fit_results->distance, det_diam );

        if( !diagJson.empty() )
        {
          json diagVis;
          diagVis["type"] = "shielding_diagram_2d";
          diagVis["title"] = "Shielding Geometry";
          diagVis["data"] = json::parse( diagJson );
          diagVis["heightPx"] = 300;
          visuals.push_back( std::move( diagVis ) );
        }
      }

      if( !visuals.empty() )
        result["_visuals"] = std::move( visuals );
    }catch( std::exception &e )
    {
      cerr << "Error creating visual content for activity_fit: " << e.what() << endl;
    }
  }
  else
  {
...
)
{
  ...
  try
  {
    ...
    // Shielding diagram
    {
      // Convert FitShieldingInfo vector to ShieldingInfo vector for the diagram
      const vector<ShieldingSourceFitCalc::ShieldingInfo> shieldings(
        fit_results->final_shieldings.begin(), fit_results->final_shieldings.end() );
  ...
  {
    cerr << "Error creating visual content for activity_fit_one_off: " << e.what() << endl;
  }
