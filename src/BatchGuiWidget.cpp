...
public:
    ...
    m_is_peaks_csv( false )
  {
    ...
    boost::function<void( void )> updateGuiCallback =
      wApp->bind( boost::bind( &InputFileWidget::set_spectrum, this, spec_meas, status_ptr ) );
...
public:
  BatchGuiAnaWidget( Wt::WContainerWidget *parent ) : Wt::WContainerWidget( parent )
  {
    addStyleClass( "BatchGuiAnaWidget" );
  }

  virtual void performAnalysis( const vector<tuple<string, string, std::shared_ptr<const SpecMeas>>> &input_files,
                                const string &output_dir ) = 0;
...
public:
  BatchGuiPeakFitWidget( Wt::WContainerWidget *parent = nullptr ) : BatchGuiAnaWidget( parent )
  {
    ...
    m_background_file_resource->fileDrop().connect( boost::bind(
      &BatchGuiPeakFitWidget::backgroundUploaded, this, boost::placeholders::_1, boost::placeholders::_2 ) );
    ...
    HelpSystem::attachToolTipOn( m_use_exemplar_energy_cal, WString::tr( "bgw-use-exemplar-energy-cal-tt" ), showToolTips );
    ...
    HelpSystem::attachToolTipOn(
      m_use_exemplar_energy_cal_for_background, WString::tr( "bgw-use-exemplar-energy-cal-for-background-tt" ), showToolTips );
    ...
    HelpSystem::attachToolTipOn( m_peak_hypothesis_threshold, WString::tr( "bgw-peak-hypothesis-threshold-tt" ), showToolTips );
    ...
    m_summary_custom_report_container->doJavaScript( "setupOnDragEnterDom(['" +
                                                     m_summary_custom_report_container->id() + "']);" );
  ...
  void perFileCustomReportUploaded( const std::string &, const std::string & )
  {
    ...
    for( const auto &file : spooled )
    {
      ...
      MiscInputFileWidget *input =
        new MiscInputFileWidget( display_name, path_to_file, should_delete, m_per_file_custom_report_container );
  ...
  void summaryCustomReportUploaded( const std::string &, const std::string & )
  {
    ...
    for( const auto &file : spooled )
    {
      ...
      MiscInputFileWidget *input =
        new MiscInputFileWidget( display_name, path_to_file, should_delete, m_summary_custom_report_container );
      input->remove_self_request().connect( boost::bind(
  ...
  virtual void performAnalysis( const vector<tuple<string, string, std::shared_ptr<const SpecMeas>>> &input_files,
                                const string &output_dir ) override
  {
    ...
    std::function<void( void )> update_gui_fcn = [results, options, close_waiting_dialog]()
    {
      ...
      } catch( nlohmann::json::parse_error &e )
      {
        const string contents =
          "<p>" + WString::tr( "bgw-json-parse-error" ).arg( string( e.what() ) ).toUTF8() + "</p>";
        WText *result_text = new WText( contents, Wt::TextFormat::XHTMLUnsafeText, dialog->contents() );
...
public:
                                ...
                                const string &output_dir ) override
  {
    ...
    shared_ptr<BatchActivity::BatchActivityFitSummary> summary_results = make_shared<BatchActivity::BatchActivityFitSummary>();
                                               ...
                                               sessionid]()
    {
      try
      {
        ...
        BatchActivity::fit_activities_in_files( exemplar_filename, exemplar, exemplar_samples, file_names, 
              input_files_meas, options, summary_results.get() );
        WServer::instance()->post( sessionid, update_gui_fcn );
      } catch( std::exception &e )
      {
        *error_msg = e.what();
  ...
  void detectorUploaded( const std::string &, const std::string & )
  {
    ...
    for( const auto &file : spooled )
    {
      ...
      try
      {
        ...
        {
          MiscInputFileWidget *input =
            new MiscInputFileWidget( display_name, path_to_file, should_delete, m_detector_file_drop );
          input->remove_self_request().connect(
  ...
  void handle_remove_detector_upload( MiscInputFileWidget *input )
  {
    delete input;
...
void BatchGuiWidget::addInputFiles( const std::vector<std::tuple<std::string, std::string, bool>> &files )
{
  for( const tuple<string, string, bool> &file : files )
  {
    ...
    InputFileWidget *input_file =
      new InputFileWidget( display_name, path_to_file, should_delete, m_input_files_container );
