...
namespace
{
  // Adapter for Wt3 pattern: bool resolveKey(key, outStr)
  ...
  public:
    ...
    {
      m_text = addNew<WText>();
      m_load = addNew<WPushButton>( WString::tr("uiw-load-btn") );
  ...
  //, m_videoTab( nullptr )
{
  ...
  auto stackUptr = std::make_unique<WStackedWidget>();
  WStackedWidget *stack = stackUptr.get();
  ...
  auto menuUptr = std::make_unique<WMenu>( stack );
  m_menu = menuUptr.get();
  ...
  welcomeContainer->setOffsets(WLength(0,WLength::Unit::Pixel));
  welcomeContainer->setMargin(WLength(0,WLength::Unit::Pixel));
  
  auto tabWUptr = std::make_unique<Wt::WTabWidget>();
  Wt::WTabWidget *tabW = tabWUptr.get();
  tabW->addStyleClass( "UseInfoTabsWidget" );
  tabW->contentsStack()->addStyleClass( "UseInfoTabsStack" );

  //The below tabs must use ContentLoading::Eager (PreLoading), or else we will get a
  ...
  auto spectrumContainerUptr = std::make_unique<WContainerWidget>();
  WContainerWidget *spectrumContainer = spectrumContainerUptr.get();
  WGridLayout *spectrumLayout = spectrumContainer->setLayout( std::make_unique<WGridLayout>() );
  spectrumLayout->setContentsMargins( 3, 5, 4, 5);
  spectrumContainer->setOverflow( Overflow::Hidden );
  // addTab takes ownership, returns raw ptr
  tabW->addTab( std::move(spectrumContainerUptr), WString::tr("uiw-tab-saved-states"), Wt::ContentLoading::Eager );

  auto samplesContainerUptr = std::make_unique<WContainerWidget>();
  WContainerWidget *samplesContainer = samplesContainerUptr.get();
  WGridLayout *samplesLayout = samplesContainer->setLayout( std::make_unique<WGridLayout>() );
  samplesLayout->setContentsMargins( 0, 0, 0, 0 );
  samplesContainer->setOverflow( Overflow::Auto );
  tabW->addTab( std::move(samplesContainerUptr), WString::tr("uiw-tab-example-spectra"), Wt::ContentLoading::Eager );
  ...
  {
    auto importContainerUptr = std::make_unique<WContainerWidget>();
    importContainer = importContainerUptr.get();
    importLayout = importContainer->setLayout( std::make_unique<WGridLayout>() );

    tabW->addTab( std::move(importContainerUptr), WString::tr("uiw-tab-import-spectra"), Wt::ContentLoading::Eager );
  }//if( m_viewer->isMobile() )
  ...
  try
  {
    ...
    m_snapshotModel = spectrumLayout->addWidget( std::make_unique<SnapshotBrowser>( manager, m_viewer, nullptr ), 0, 0 );
#endif
    
    const string docroot = wApp->docRoot();
    
    ///-----Sample -----
    m_messageModelSample = std::make_shared<Wt::WStandardItemModel>( 0, 3 );

    // Helper lambda to append a row to the sample model
    auto appendSampleRow = [this]( const string &label, const string &path, SpecUtils::ParserType ptype ){
      auto col0 = std::make_unique<Wt::WStandardItem>( label );
      auto col1 = std::make_unique<Wt::WStandardItem>( path );
    ...
    appendSampleRow( "Ba-133 (16k bin N42)",
                     ...
                     SpecUtils::ParserType::N42_2006 );
    appendSampleRow( "Passthrough (16k bin, 8 det, 1064 meas N42)",
                     SpecUtils::append_path(docroot, "example_spectra/passthrough.n42"),
                     SpecUtils::ParserType::N42_2006 );
    appendSampleRow( "Background (16k bin N42)",
    ...
    for( const pair<string, string> &descript_filename : ref_spectra )
    {
      ...
      for( const pair<string, string> dettype_path : ref_detectors )
      {
        ...
        appendSampleRow( descrip, fullpath, SpecUtils::ParserType::Auto );
    ...
    m_tableSample->setOffsets( WLength(0, WLength::Unit::Pixel), Wt::Side::Left | Wt::Side::Top );
    m_tableSample->setColumnResizeEnabled(true);
    m_tableSample->setRowHeight( WLength(30,WLength::Unit::Pixel) );
    m_tableSample->setColumnAlignment(0, Wt::AlignmentFlag::Left);
    //m_tableSample->setHeaderAlignment(0, Wt::AlignmentFlag::Center);
    if( m_viewer->isPhone() )
    ...
    if( importContainer )
    {
      const string msg0 = WString::tr("uiw-tap-choose-file").toUTF8();
      WText *desc = importLayout->addWidget( std::make_unique<WText>( msg0 ), 0, 0, Wt::AlignmentFlag::Center );
      desc->setTextAlignment( Wt::AlignmentFlag::Center );
      ...
      const string msg2 = WString::tr("uiw-open-file-instructions").toUTF8();
      importLayout->addWidget( std::make_unique<WText>( msg2 ), 2, 0, Wt::AlignmentFlag::Bottom );
    }//if( importContainer )
  ...
  //----------Add left menu --------
  auto itemUptr = std::make_unique<SideMenuItem>( (m_viewer->isPhone() ? WString() : WString::tr("uiw-welcome")), std::move(welcomeContainerUptr) );
...
*/
  ...
  if( m_viewer->isMobile() )
  {
    ...
    {
      WAnchor *anchor = item->anchor();
      anchor->addNew<WImage>( "InterSpec_resources/images/phone_tips.svg" )->addStyleClass( "WhiteIcon" );
    }
  }else if( m_viewer->isDesktop() )
  {
    auto controlContainerUptr = std::make_unique<WContainerWidget>();
    WContainerWidget *controlContainer = controlContainerUptr.get();
    controlContainer->setOverflow(Overflow::Hidden);
    WGridLayout *controlLayout = controlContainer->setLayout( std::make_unique<WGridLayout>() );
    controlLayout->setContentsMargins(0, 0, 0, 0);

    Wt::WTabWidget *controlTab = controlLayout->addWidget( std::make_unique<Wt::WTabWidget>(), 1, 0 );

    const WBorder border( Wt::BorderStyle::Solid, Wt::BorderWidth::Thin, Wt::StandardColor::Gray );
    controlTab->contentsStack()
              ->decorationStyle()
              .setBorder( border, Wt::Side::Bottom | Wt::Side::Right | Wt::Side::Left );
    ...
    std::string controlInfo = "";
    resolveKey( m_resourceBundle, "desktop-mouse-interactions-info", controlInfo );
    ...
    try
    {
          ...
          iter != result.end(); ++iter )
      {
        ...
        if( !desc_id.isNull() )
          resolveKey( m_resourceBundle, static_cast<std::string>( desc_id ), descstr );

        auto tabContainerUptr = std::make_unique<WContainerWidget>();
        WContainerWidget *samplesContainer = tabContainerUptr.get();
        samplesContainer->addStyleClass("WelcomeControlsTabContents");

        if( !imagestr.empty() )
        {
          WImage *img = samplesContainer->addNew<WImage>( imagestr.toUTF8() );
        ...
        if( !descstr.empty() )
        {
          WText *txt = samplesContainer->addNew<WText>( descstr, Wt::TextFormat::UnsafeXHTML );
          txt->setStyleClass("img-centered");
        } //descstr.empty()

        controlTab->addTab( std::move(tabContainerUptr), titlestr, Wt::ContentLoading::Lazy );
...
*/
  ...
  auto textUptr = std::make_unique<WText>( msg );
  WText *text = textUptr.get();
  ...
  if( m_viewer->isPhone() )
  {
    rightLayout = welcomeContainer->setLayout( std::make_unique<WGridLayout>() );
    ...
    rightLayout->addWidget( std::move(textUptr), 0, 0 );
    rightLayout->addWidget( std::move(tabWUptr), 1, 0 );
    rightLayout->setRowStretch( 1, 1 );

    text = rightLayout->addWidget( std::make_unique<WText>( mobilePostcriptText, Wt::TextFormat::UnsafeXHTML ), 2, 0 );
    text->setInline( false );
  ...
  }else
  {
    ...
    auto moreUptr = std::make_unique<WAnchor>( WLink(), WString::tr("uiw-license-and-terms") );
    WAnchor *more = moreUptr.get();
    ...
    //regular locations
    auto more2Uptr = std::make_unique<WAnchor>( WLink(), WString::tr("uiw-more-info") );
    WAnchor *more2 = more2Uptr.get();
    more = more2;  // reassign for use below
  ...
  if( showAgainCallback )
  {
    ...
    {
      showAgainCb = rightLayout->addWidget( std::make_unique<WCheckBox>( WString::tr("uiw-show-at-start-cb") ), rightLayout->rowCount(), 0 );
      showAgainCb->addStyleClass( "CbNoLineBreak" );
    }else
    {
      // Put CB into header
      showAgainCb = bottom->addNew<WCheckBox>( WString::tr("uiw-show-at-start-cb") );
    }
    showAgainCb->setFloatSide( Wt::Side::Left );
    ...
    if( m_viewer->isMobile() )
    {
      ...
      showAgainCb->setMargin( 10, Wt::Side::Right );
      showAgainCb->setMargin( 3, Wt::Side::Top );
    }//isPhone
  }//if( !force )
...
SideMenuItem * UseInfoWindow::makeItem( const WString &title, const string &resource)
{
  ...
  wUptr->addStyleClass( "UseInfoItem" );
...
SideMenuItem *UseInfoWindow::makeTextItem( const Wt::WString &title, const std::string &resource )
{
  ...
  std::function<void(WContainerWidget *)> f = [this, resourceCopy](WContainerWidget *w){
    textItemCreator( resourceCopy, w );
  };
...
void UseInfoWindow::textItemCreator( const std::string &resource, Wt::WContainerWidget *parent )
{
  ...
  try
  {
    ...
    // Create intro
    WContainerWidget *into_div = parent->addNew<WContainerWidget>();
    into_div->addStyleClass( resource_name + "-intro" );

    std::string intro_txt;
    resolveKey( bundle, "intro", intro_txt );
    WText *intro = into_div->addNew<WText>( WString::fromUTF8(intro_txt) );
    intro->setInline( false );
    intro->addStyleClass( resource_name + "-intro-txt" );

    // Create a table of contents
    WContainerWidget *into_toc = into_div->addNew<WContainerWidget>();
    into_toc->setList( true );
    into_div->addStyleClass( resource_name + "-toc" );

    // Add the contents
    WContainerWidget *subjects = parent->addNew<WContainerWidget>();
    subjects->addStyleClass( resource_name + "-subjects" );

    for( const pair<string,string> &key_title : topic_keys )
    {
      WContainerWidget *w = subjects->addNew<WContainerWidget>();
      w->addStyleClass( resource_name + "-subject" );

      WContainerWidget *title = w->addNew<WContainerWidget>();
      title->addStyleClass( resource_name + "-subject-title" );
      WPushButton *up_btn = title->addNew<WPushButton>();
      ...
      title->addNew<WText>( key_title.second );

      std::string content;
      resolveKey( bundle, key_title.first, content );

      //cout << "key: " << key_title.first << " --> content: " << content << endl;

      WText *sub = w->addNew<WText>( content );
      sub->addStyleClass( resource_name + "-subject-content" );

      w->setObjectName( key_title.first );

      // Add to the TOC
      WContainerWidget *toc_item = into_toc->addNew<WContainerWidget>();
      ...
      WText *link = toc_item->addNew<WText>( key_title.second );
      link->addStyleClass( resource_name + "-toc-item LinkBtn" );
    }//for( const pair<string,string> &key_title : topic_keys )
  }catch( std::exception &e )
  {
    parent->addNew<WText>( WString::tr("uiw-error-loading-faq").arg(e.what()) );
  }//try catch
...
{
  if( event.button() != Wt::MouseButton::Left || !index.isValid() )
    return;
...
void UseInfoWindow::loadSample( const Wt::WModelIndex index )
{
  ...
  WString val = Wt::cpp17::any_cast<WString> (m_messageModelSample->data(index.row(), 1, Wt::ItemDataRole::Display));
  SpecUtils::ParserType parserType = Wt::cpp17::any_cast<SpecUtils::ParserType> (m_messageModelSample->data(index.row(), 2, Wt::ItemDataRole::User));
...
SideMenuItem::SideMenuItem( const WString &text, std::unique_ptr<WWidget> contents )
: WMenuItem( text, std::move(contents), Wt::ContentLoading::Lazy )
{
} //SideMenuItem::SideMenuItem( const WString &text, std::unique_ptr<WWidget> contents )
