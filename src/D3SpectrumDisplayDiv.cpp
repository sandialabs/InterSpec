...
void D3SpectrumDisplayDiv::saveChartToImg( const std::string &filename, const bool asPng )
{
  ...
  captureChartImage( format, 0, {}, {}, {},
    [this, filename]( std::string b64, std::string mime, int w, int h ){
      handleChartImageForDownload( filename, std::move( b64 ), std::move( mime ), w, h );
    } );
                                                         ...
                                                         int, int )
{
  ...
  {
    m_downloadResource = new Wt::WMemoryResource();
    addChild( std::unique_ptr<Wt::WMemoryResource>( m_downloadResource ) );
  }

  m_downloadResource->setMimeType( mimeType );
  m_downloadResource->setData( data );
  m_downloadResource->suggestFileName( Wt::WString::fromUTF8( filename ), Wt::ContentDisposition::Attachment );
                                               ...
                                               ImageCaptureCallback callback )
{
  ...
  if( !m_imageCapturedJS )
  {
    ...
    m_imageCapturedJS->connect( this, [this]( std::string b64, std::string mime,
                                              int w, int h, int captureId ){
      handleImageCaptured( std::move( b64 ), std::move( mime ), w, h, captureId );
    } );
