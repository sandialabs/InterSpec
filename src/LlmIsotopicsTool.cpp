...
nlohmann::json stateToJsonDetailed( const RelActCalcAuto::RelActAutoGuiState &state )
{
  ...
  auto curveToJson = [](const RelActCalcAuto::RelEffCurveInput &curve, bool include_index, size_t index) -> json {
    ...
    if( is_physical )
    {
      ...
      // TODO: deal with Cheby correction functions, or at least make sure we cant get here with one.
      assert( curve.phys_model_corr.corr_fcn != RelActCalc::PhysModelCorrFcn::Chebyshev );
      curve_json["use_hoerl"] = (curve.phys_model_corr.corr_fcn == RelActCalc::PhysModelCorrFcn::Hoerl);
...
)
{
  ...
  std::unique_ptr<RelActCalcAuto::RelActAutoGuiState> state_ptr = spec->getRelActAutoGuiState();
  ...
  for( const string &filepath : preset_files )
  {
    ...
    try
    {
      ...
      if( base_node )
      {
        ...
        file_state.deSerialize( base_node );
...
)
{
  ...
  try
  {
    const shared_ptr<const PeakFitDetPrefs> fitPrefs = spec->peakFitDetPrefs();
    ...
    // Add HTML report visual for display in LlmInteractionDisplay (not sent to LLM)
    try
    {
      ostringstream html_strm;
      solution.print_html_report( html_strm );
      const string htmlContent = html_strm.str();
      ...
      if( !htmlContent.empty() )
      {
        ...
        json htmlVis;
        htmlVis["type"] = "html";
        htmlVis["title"] = "Isotopics Analysis Report";
        htmlVis["description"] = "Detailed results including relative efficiency curves and peak fits";
        htmlVis["data"] = htmlContent;
        htmlVis["heightPx"] = 500;
        visuals.push_back( std::move( htmlVis ) );
        result["_visuals"] = std::move( visuals );
      }
    }catch( exception &e )
    {
      cerr << "Error creating HTML report visual for isotopics: " << e.what() << endl;
    }
...
namespace
{
    ...
    const nlohmann::json &shield_json )
  {
    ...
    if( shield_json.contains( "material" ) )
    {
      ...
      const std::shared_ptr<const Material> mat = materialDB->material( mat_name );
      if( !mat )
  ...
  InterSpec* interspec )
{
  ...
  if( params.contains( "use_hoerl" ) )
  {
    ...
    const bool use_hoerl = params.at( "use_hoerl" ).get<bool>();
    curve.phys_model_corr.corr_fcn = use_hoerl ? RelActCalc::PhysModelCorrFcn::Hoerl
                                               : RelActCalc::PhysModelCorrFcn::None;
    changes.push_back( string( "use_hoerl=" ) + (use_hoerl ? "true" : "false") );
  }
  ...
  if( params.contains( "self_attenuation" ) )
  {
    ...
    {
      curve.phys_model_self_atten = parseShieldingFromJson( self_atten_json );
  ...
  if( params.contains( "external_attenuation" ) )
  {
    ...
    if( ext_atten_json.is_array() )
    {
      ...
      {
        auto ext = parseShieldingFromJson( ext_json );
  ...
  state.options.check_same_corr_fcn_and_external_shielding_specifications();
