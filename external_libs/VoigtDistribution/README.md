...
Also implements a pseudo-Voigt distribution, for speed.
...
// Sample Voigt distribution with exponential tail
double pdf_val = voigt_exp_tail::voigt_exp_pdf(x, mean, sigma, gamma, tail_ratio, tail_slope);
```
...
Implementation of the exact Voigt CDF proved to be challenging, so instead the PDF is integrated
using Gauss-Legendre quadrature for the `voigt_exp_tail::voigt_exp_integral(...)` functions.
This also means a `voigt_exp_tail::voigt_exp_coverage_limits(...)` is not yet implemented.
However, the pseudo-Voigt distribution does have a stable CDF, so these limitations are not
applicable to it.