...
public:
   ...
   {
     //assert( min != max );
      if( min == max )  //wcjohns mode
        throw std::runtime_error( "MinuitParameter: min == max for " + name );
   ...
   void SetLimits(double low, double up) {
     //assert(low != up);
     if( low == up )  //wcjohns mode
       throw std::runtime_error( "MinuitParameter: low == up for " + fName );
